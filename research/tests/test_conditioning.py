...
class PitchConditioningTest(unittest.TestCase):
    ...
    def test_note_controls_have_explicit_f0_loudness_and_gate_channels(self):
        ...
        self.assertEqual(controls.shape, (1, 3, 4))
    ...
    def test_invalid_conditioning_is_rejected_at_the_model_boundary(self):
        with self.assertRaisesRegex(ValueError, "shape"):
            validate_conditioning(np.zeros((1, 2, 8), dtype=np.float32))
        invalid = np.zeros((1, 3, 8), dtype=np.float32); invalid[:, 1] = 2
        with self.assertRaisesRegex(ValueError, "loudness and gate"):
