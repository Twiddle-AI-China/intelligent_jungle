...
过 `timbre.net`，量两两余弦（`tools/collapse_probe.py`，在 Spark 上跑）：
...
源码 `/home/jyhu/MidiBrave/src/midibrave/model.py:212`（Spark，只读）：
...
训练用的： laion-clap 1.1.7 / HTSAT-base / enable_fusion=False
           /data/model_weights/laion-clap/music_audioset_epoch_15_esc_90.14.pt   (Spark)
...
缓存在 Spark `/data/midibrave/cache/serum_strict_1822/clap/`。直接用缓存 = 零重算、零漂移，
...
| `tools/collapse_probe.py` | 塌陷判定（Spark，纯 numpy） |
| `tools/descriptors.py` | 1822 preset 的 DSP 画像（Spark，纯 numpy） |
...
- checkpoint sha256 `f6a37061e1883dd44d7c3462c240f92eb8510db893779e2d8d201e8999111508`
  （`/data/model_weights/midiBrave/midibrave-full-c9-phase1-step-000075365.pt`，就在 Spark 本机）
