...
- **是 state_dict，不是 TorchScript。** 没有模型类就加载不了。源码 `/home/jyhu/MidiBrave` 在 Spark 本机。
- checkpoint 顶层键：`format=3, phase=1, step=75364, generator_updates=75365,
  discriminator_updates=0, world_size=8（8 卡训练；Spark 单卡等效 wall ≈16×，见 00-equivalent-compute-basis.md）, epoch=5, model, optimizer, scaler, rng_by_rank`。
