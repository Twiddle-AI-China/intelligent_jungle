...
目标声音投影：群体越紧密，Voice 越稳定、集中；群体散开时，粗糙度、噪声感和立体宽度增加。当前 Web 版会计算这些状态，但尚未完整消费。

## 3. Alignment

每只鸟逐渐靠近同群邻居的平均速度方向。它解决“动作能否在群体里传播”。

目标声音投影：平均方向决定音色轨迹方向，平均速度决定能量与变化速度。当前 Web 版只有平均水平方向部分控制 brightness，不能宣称声音已完整跟随二维转向。

## 4. Separation

任意两只鸟距离过近时互相避让，包括不同声音群之间的鸟。它解决“对象是否重叠、Voices 是否糊在一起”。

目标声音投影：不同群的空间分离产生声像和频谱分离；持续靠近或绕障碍时增加瞬态和频谱避让。当前只有群心 X→声像成立，频谱避让尚未实现。

## 5. 障碍

障碍产生局部排斥场，但不被称为第四条 Boids 规则。每只鸟记录绕行压力；Flock 会汇总该压力。当前压力通过运动和 pulse 间接影响 Web 声音，直接的瞬态、粗糙度和噪声映射仍待实现。
