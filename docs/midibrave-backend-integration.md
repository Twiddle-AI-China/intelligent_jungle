...
> （允许按声部混用，亮部盲听后定）；③ **首要阻塞 = 请 jyhu 把两个 .pt 从 Octopus 同步到 Spark
...
- **发布缺口（需协同）**：两个 .pt 实体尚未同步到 Spark `/data/model_weights/midiBrave`（目录为空）；训练/评测产物在 Octopus。
...
- 两者同 config：`/workspace/MidiBrave/configs/quality150_c9_optimized.yaml`，同 Phase 1；训练/评测在 Octopus（58.216.118.227，SLURM job 906），Spark 只拉取评测产物（finalize 脚本只同步 dashboard 数据，未同步 .pt）。
...
- 是否支持音符序列输入、是否流式（causal）未知（config 在 Octopus）→ 开放问题 Q2。
...
- **Q2** 架构与流式能力：`/workspace/MidiBrave` config 在 Octopus——RAVE 系 or 扩散？单音 or 音符序列？决定 A 是否可行（问 jyhu 最快）。
