...
- Web MVP 已加载这 12 个真实 BRAVE 音频端点，页面显示 `BRAVE 神经声音 · 1364498c`。
...
1. 由用户按 5 分钟体验指南判断加鸟、障碍和引导的声音因果是否清楚。
2. 对 3 个 Species 和轨迹两端做盲听；不可区分的方向不命名、不进入正式 atlas。
3. 原生侧使用后台 decoder worker 与音频 ring buffer 吸收推理尖峰，再重新跑 30 分钟闸门。
