...
实现状态（2026-07-22）：已落地首版。四张写实 `bg-*.jpg` 已退出运行时配置，替换为项目自有透明 SVG：塔罗双层边框、星图与春芽/夏蕨/秋穗/冬枝季节符号；只使用 `ink/accent`，中央留给树干与 Sequence。renderer 的季节交叉淡化、全日线性明暗、Linux Antiquity 日月与正拍脉冲保持原逻辑。
