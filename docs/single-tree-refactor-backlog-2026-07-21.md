...
下一轮音景/混音需求池（2026-07-22，开发中）：

- **P0 环境底噪**：接入用户提供的 302.04 秒、48kHz stereo 森林录音。受浏览器 autoplay 限制，进入页点击启用音频后才可开始；正常播放保持低电平，真实乐器 RMS 出现时快速 duck，停顿后慢回，暂停时只淡出乐器而让环境声升起。
- **P0 树干去控件**：Canvas 不再绘制或命中三组年轮；每个声部在右侧抽屉统一为六滑杆：`Volume / EQ Low / EQ Mid / EQ High / Reverb / Ping-pong`。Ping-pong 时值随 Master BPM，不再暴露另一套 tempo。
- **P0 Bass 低音域**：当前高张力下 Bass 候选可达 MIDI 84，约 99.3% 候选高于 E3，属于音池窗口与发声 register 双重问题。Bass 树新增低枝概率属性，Agent/Sequence 同源消费；Bass 音池固定守低窗口且发声整体再降八度。
- **P1 声部 Agent 混音**：决策主体是每棵树/声部 flock，不是多只鸟并发抢写同一总线。EQ、Reverb、Ping-pong 的每日限幅小步计入音色探索；Volume 只响应四轨电平平衡与削波护栏，计入整体和谐。USER 接管冻结 Agent 对六项的写入。
- **P1 音色林地**：用户界面删除 `kNN / PCA / 邻居 k / 高阶主成分` 等研究术语和调参，只保留可拖动音色地图、当前位置试听与探索记忆反馈；安全 kNN 与 PCA 协议继续作为内部实现，不再成为用户选择题。
