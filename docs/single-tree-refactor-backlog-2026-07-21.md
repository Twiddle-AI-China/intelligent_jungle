...
候选循环：Day T 白天记录各 Bird Agent 的可观察行为；Day T 夜晚由 Master 结合环境状态与四声部整体和谐度，结算三维资源 `体力值 / 生命值 / 捕获量`；在 T+1 白天前向各 Bird Agent 下发资源变化与行动倾向。夜晚默认复奏当天白天的音乐材料，形成记忆与重复，而不是继续生成同等密度的新变奏。
...
当前状态：**仅进入方案评审，不授权重写 economy/world**。优先做离线 shadow scoring：同一批 Day T 日志同时跑现行规则和三维 Master 结算，比较可解释性、稳定性与是否产生更清晰的 T+1 行动差异，再决定迁移范围。
