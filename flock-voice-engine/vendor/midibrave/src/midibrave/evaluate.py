...
def generate_grid(model: MidiBrave, dataset: PairDataset, config: Config,
    ...
    for record, category in selected:
        clap = torch.from_numpy(np.load(
            Path(config.data.cache_root) / "clap" / f"{record.sample_id}.npy"
...
def evaluate(config_path: str, checkpoint_path: str, output_path: str,
    ...
    for raw_batch in loader:
        ...
        metrics.add("self_mr_stft", stft(self_audio, audio_a))
        metrics.add("cross_mr_stft", stft(cross_audio, audio_b))
        for prefix, prediction, target in (
                ("self", self_audio, audio_a), ("cross", cross_audio, audio_b)):
            ...
            metrics.add(f"{prefix}_rms_error_db", (rms_db(prediction) - rms_db(target)).abs())

        branch_pitch: dict[str, tuple[list[list[float]], list[float | None],
                                      list[list[float]]]] = {}
        for prefix, generated, target, notes, velocities, sample_ids in (
                ("self", self_audio, audio_a, note_a, batch["velocity_a"][:current],
                 raw_batch["sample_id_a"][:current]),
                ("cross", cross_audio, audio_b, note_b, batch["velocity_b"][:current],
        ...
        prediction_delta = rms_db(cross_audio) - rms_db(self_audio)
