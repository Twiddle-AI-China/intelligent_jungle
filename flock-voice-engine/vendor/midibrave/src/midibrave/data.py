...
class SampleRecord:
    ...
    def group_id(self) -> str:
        return f"{self.source_id}|{self.preset_id}|{self.articulation_id}"
...
def load_manifest(path: str | Path) -> list[SampleRecord]:
    ...
    for row in read_jsonl(path):
        record = SampleRecord(**{key: row.get(key) for key in SampleRecord.__dataclass_fields__})
...
def _eligible_pitch_offset_frames(valid: np.ndarray, cfg: DataConfig) -> np.ndarray:
    if cfg.window_samples % cfg.pitch_hop_length:
        raise ValueError("window_samples must be divisible by pitch_hop_length")
    window_frames = cfg.window_samples // cfg.pitch_hop_length
...
def _pitch_cache_status(path: Path, record: SampleRecord,
    ...
    try:
        with np.load(path, allow_pickle=False) as pitch:
            ...
            return True, bool(len(_eligible_pitch_offset_frames(derived_valid, cfg)))
    except (OSError, ValueError, EOFError):
        return False, False
...
def validate_manifest(records: list[SampleRecord], cfg: DataConfig, manifest_path: str | Path) -> dict[str, Any]:
    root = dataset_root(cfg, manifest_path)
    ...
    for record in records:
        ...
        if record.velocity not in cfg.velocities:
            errors.append(f"velocity: {record.sample_id}: {record.velocity}")
        if not 0 <= record.midi_note_sent <= 127:
            errors.append(f"sent MIDI range: {record.sample_id}: {record.midi_note_sent}")
        if record.transpose_semitones != record.midi_note - record.midi_note_sent:
            errors.append(
        ...
        if cfg.require_pitch_cache:
            pitch_path = cache_root / "pitch" / f"{record.sample_id}.npz"
            clap_path = cache_root / "clap" / f"{record.sample_id}.npy"
    ...
    for group, members in by_group.items():
        ...
        if not _supports_all_pair_modes(members):
            errors.append(f"missing sparse pair mode coverage: {group}")
        coverage.append({
            "samples": len(members),
...
def cache_audio_regions(manifest: str | Path, cache_root: str | Path, window_samples: int,
                        root: str | Path | None = None, shard_index: int = 0,
                        shard_count: int = 1) -> dict[str, int]:
    manifest = Path(manifest).resolve()
    root = Path(root).resolve() if root is not None else manifest.parent
    ...
    for record in records:
        destination = output / f"{record.sample_id}.npz"
        try:
        ...
        start, end = active_region(audio, window_samples)
        _atomic_save_npz(
...
def cache_clap_embeddings(
    ...
    records = _sharded(load_manifest(manifest), shard_index, shard_count)
    for record in records:
        destination = output / f"{record.sample_id}.npy"
...
def cache_pitch_features(manifest: str | Path, cache_root: str | Path, device: str,
                         cfg: DataConfig, root: str | Path | None = None,
    ...
    for record in records:
        ...
        if structurally_valid:
            ...
            continue
        waveform = load_audio((root / record.audio_path).resolve(), record.sample_rate)
...
def finalize_cache_manifest(manifest: str | Path, cfg: DataConfig,
    ...
    for group_id, members in sorted(by_group.items()):
        ...
        elif not _supports_all_pair_modes(members):
            dropped_groups[group_id] = "missing_pair_mode"
        else:
...
class PairDataset(Dataset[dict[str, Any]]):
    ...
    def __init__(self, cfg: DataConfig, seed: int) -> None:
        ...
        self.root = dataset_root(cfg, self.manifest)
        ...
        self.mode_anchors: dict[str, list[SampleRecord]] = {
            mode: [] for mode in set(self.PAIR_SEQUENCE)
        }
        for record in self.records:
            for mode in self.mode_anchors:
                if self._candidates(record, mode):
                    self.mode_anchors[mode].append(record)
        missing_modes = [mode for mode, anchors in self.mode_anchors.items() if not anchors]
        if missing_modes:
            raise ValueError(f"dataset cannot construct pair modes: {sorted(missing_modes)}")
    ...
    def __len__(self) -> int:
        return len(self.records) * self.cfg.repeats
    ...
    def _window(self, record: SampleRecord, rng: random.Random) -> dict[str, torch.Tensor]:
        audio = load_audio((self.root / record.audio_path).resolve(), self.cfg.sample_rate)
        reference_rms_db = self.velocity_reference_rms_db.get(record.sample_id)
        if reference_rms_db is None:
            ...
            self.velocity_reference_rms_db[record.sample_id] = reference_rms_db
        pitch_file = self.cache_root / "pitch" / f"{record.sample_id}.npz"
        ...
        if pitch_file.is_file():
            ...
            if not len(eligible_offsets):
                raise RuntimeError(f"no eligible pitch-supported window: {record.sample_id}")
            ...
            start, end = active_region(audio, self.cfg.window_samples)
            first_frame = math.ceil(start / hop)
            last_frame = math.floor((end - self.cfg.window_samples) / hop)
            if last_frame < first_frame:
                raise RuntimeError(f"active interval cannot align to pitch frames: {record.sample_id}")
            offset_frame = (first_frame if last_frame == first_frame
                            else rng.randint(first_frame, last_frame))
        offset = offset_frame * hop
        window = audio[offset : offset + self.cfg.window_samples]
        if len(window) != self.cfg.window_samples:
            raise RuntimeError(f"bad window length: {record.sample_id}")
        ...
        if pitch_cache is None:
            ...
            f0 = pitch_cache["f0"][offset_frame:stop].astype(np.float32)
            confidence = pitch_cache["periodicity"][offset_frame:stop].astype(np.float32)
            valid = derived_valid[offset_frame:stop].astype(np.bool_)
            if len(f0) != pitch_frames:
                raise RuntimeError(f"pitch cache is shorter than crop: {record.sample_id}")
        return {
    ...
    def _clap(self, record: SampleRecord) -> torch.Tensor:
        path = self.cache_root / "clap" / f"{record.sample_id}.npy"
        if not path.is_file():
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        mode = self.PAIR_SEQUENCE[index % len(self.PAIR_SEQUENCE)]
        anchors = self.mode_anchors[mode]
        ...
        return {
            ...
            "pair_mode": mode,
            "midi_note_sent_a": torch.tensor(a.midi_note_sent, dtype=torch.long),
            "midi_note_sent_b": torch.tensor(b.midi_note_sent, dtype=torch.long),
            "transpose_semitones_a": torch.tensor(a.transpose_semitones, dtype=torch.long),
            "transpose_semitones_b": torch.tensor(b.transpose_semitones, dtype=torch.long),
        }
