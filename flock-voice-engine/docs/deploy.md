...
| 服务端口 | **8090**（唯一） |
| SSH | `ssh rolf@192.168.9.140`（密码见本地凭据记录(不入库)；或用各自账号登 Spark 本机） |
...
```bash
ssh rolf@192.168.9.140          # 密码见本地凭据记录(不入库)，sshpass 没装，脚本里用 expect
...
expect { -re {assword:} { send "<SSH_PASSWORD>\r"; exp_continue } eof }'
```
