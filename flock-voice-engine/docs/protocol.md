...
数据来自发送循环里顺手记的快照,过期上限约一个生产块（约 **92.88 ms**）。
...
服务端同时把 p50/p95/max 的 `renderMs` 落盘到 `logPath`（每
`40 × 4096 / 44100 ≈ 3.72 s` 一行，见 `server/app.py` 的
`LOAD_LOG_EVERY_BLOCKS`），那是给事后排查用的，接口本身只报最新快照。
