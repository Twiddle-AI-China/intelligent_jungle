...
W = np.load("/home/rolf/staging/timbre_net.npz")
...
def adapter(clap, pre_tanh=False):
    """完全复刻 TimbreAdapter.forward: net(F.normalize(clap))"""
    x = l2(np.asarray(clap, np.float64))
    x = layernorm(x, W["0.weight"], W["0.bias"])
    x = silu(x @ W["1.weight"].T + W["1.bias"])
    x = x @ W["3.weight"].T + W["3.bias"]
...
for f in files:
    ...
    if m: by_preset[m.group(1)].append(f)
...
for i, (pid, fl) in enumerate(sorted(by_preset.items())):
    ...
    E = np.stack([np.load(os.path.join(CLAP_DIR, f)).astype(np.float64).ravel() for f in sel])
...
Zp = adapter(M)                     # (P,128) tanh 后
Hp = adapter(M, pre_tanh=True)      # (P,128) tanh 前
...
np.savez("/home/rolf/staging/collapse_probe.npz", presets=np.array(presets),
         Z=Zp.astype(np.float32), H=Hp.astype(np.float32), M=M.astype(np.float32))
print("\n已存 /home/rolf/staging/collapse_probe.npz")
