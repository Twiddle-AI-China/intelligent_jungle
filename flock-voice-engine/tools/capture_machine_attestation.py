...
try:
    from validate_phase5_acceptance import canonical, canonical_machine_addresses
...
def capture(output: Path, normal_profile: Path, burst_profile: Path,
            ...
            memory_path: Path = Path("/proc/meminfo")) -> dict:
    ...
    try:
        ...
        available_line = next(line for line in memory.decode().splitlines()
                              if line.startswith("MemAvailable:"))
        available_memory_bytes = int(available_line.split()[1]) * 1024
        ...
        if (not driver_version or not gpu_model or torch_value.get("available") is not True
            ...
            raise ValueError
    except (UnicodeError, json.JSONDecodeError, StopIteration, ValueError, IndexError, KeyError) as exc:
        raise CaptureError("MACHINE_ATTESTATION_CAPTURE_FAILED") from exc
