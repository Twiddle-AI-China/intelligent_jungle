...
sys.path.insert(0, '/home/rolf/projects/flock-voice-engine/vendor/midibrave/src')
...
anchors = json.load(open('/home/rolf/staging/anchors_partial.json'))
names = [a['id'] for a in anchors]
ZA = np.array([a['z_timbre'] for a in anchors], dtype=np.float32)
print("anchors:", names)

@torch.no_grad()
def render(zs, note=60, vel=127, bs=8):
    outs = []
...
def logmel(x, n_fft=2048, hop=512):
...
for k in range(len(ZA)):
    a = render([ZA[k]], note=60)[0]; b = render([ZA[k]], note=61)[0]
    ref.append(dist(a, b))
...
for i, j in pairs:
    ...
    dd = [dist(au[k], au[k+1]) for k in range(STEPS-1)]
    total = dist(au[0], au[-1])
    ...
    rows.append(dict(pair=f"{names[i]}->{names[j]}", zdist=float(np.linalg.norm(ZA[j]-ZA[i])),
                     zstep=zstep, d_mean=float(np.mean(dd)), d_max=float(np.max(dd)),
                     total=total, flat_mid_over_end=float(np.max(flat)/max(max(flat[0],flat[-1]),1e-9)),
                     rms_dip=float(np.min(rms)/max(np.mean([rms[0],rms[-1]]),1e-9))))
    ...
    print(f"  中段谱平坦度/端点={rows[-1]['flat_mid_over_end']:.2f}  RMS 最低/端点均值={rows[-1]['rms_dip']:.2f}")
    # 每单位 z 距离带来的听感变化
...
json.dump(dict(rows=rows, ref_semitone=ref_semitone, sensitivity=sens), open(f'{OUT}/roam_metrics.json','w'), indent=1)

# ---- 渲染可听素材：锚点巡回，三种速率 ----
def roam_wav(path, per_note_z, note_seq, vel=127):
    au = render(per_note_z, vel=vel) if len(set(note_seq))==1 else None
    segs = []
    for k, (z, n) in enumerate(zip(per_note_z, note_seq)):
        a = render([z], note=int(n), vel=vel)[0]
...
for rate_name, n_between in (("slow_8", 8), ("med_3", 3), ("jump_0", 0)):
    ...
    roam_wav(f'{OUT}/roam_{rate_name}.wav', zs, ns)
    print(f"  {rate_name}: 每音 Δz = {np.median([np.linalg.norm(zs[k+1]-zs[k]) for k in range(len(zs)-1)]):.3f}")
