...
(function (global) {
  ...
  function createClient(userOptions) {
    ...
    async function ensureWorklet() {
      ...
      try {
        if (options.workletUrl && options.workletUrl !== SCRIPT_DIR_WORKLET
            && options.workletUrl !== './pcm-player-worklet.js') {
          throw new Error('production worklet URL is fixed');
        }
        await context.audioWorklet.addModule('./pcm-player-worklet.js');
      } catch (error) {
        throw new Error('加载 pcm-player-worklet.js 失败：' + error.message);
      }
    ...
    async function connect(url) {
      ...
      if (options.split) {
        try {
          const status = await fetch('/api/decoder-status').then((r) => r.json());
    ...
    async function probeStatus() {
      ...
      try {
        const response = await fetch('/api/decoder-status', { mode: 'cors' });
